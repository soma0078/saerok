...
## CSS 파일 임포트 시 TypeScript 오류

**증상**

`src/app/_layout.tsx`의 `import "../global.css"` 라인에서:

```
부수 효과 가져오기에 대한 모듈 또는 형식을 찾을 수 없습니다.
```

**원인**

TypeScript가 `.css` 확장자를 모듈로 인식하지 못함. Metro 번들러는 런타임에 정상 처리하지만 TS 타입 체커는 `.css`에 대한 타입 선언이 없으면 오류를 발생시킴. 빌드 오류가 아니고 타입 체킹 오류.
...
`tsconfig.json`의 `include: ["src/**/*"]`에 자동 포함됨. 이후 VS Code에서 `Cmd+Shift+P` → `TypeScript: Restart TS Server`.
